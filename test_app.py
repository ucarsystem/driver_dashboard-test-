    ...
    if company_input and user_id_input and user_name_input:
        ...
        if not valid_user.empty:
            ...
            if not filtered.empty:
                ...
                col1.markdown(f"<div style='text-align: center; font-size: 14px; font-weight: bold;'>{int(month_input)}월 등급</div><div style= 'text-align : center; font-size: 100px; font-weight: bold; color: {grade_text_color}; text-shadow: 2px 2px 3px rgba(0,0,0,0.2); margin-top: 10px; line-height: 1;'>{this_grade}</div>", unsafe_allow_html=True)
                col2.markdown(f"""
                <div style='font-size: 14px; color: gray; margin-top: 40px;'>달성률</div>
                <div style='font-size: 26px; font-weight: bold;'>{round(this_percent * 100)}%</div>
                """, unsafe_allow_html=True)
                col3.markdown(f"""
                    <div style='font-size: 14px; color: gray; margin-top: 60px;'>공회전</div>
                    <div style='font-size: 26px; font-weight: bold;'>{round(this_idle * 100)}%</div>
                """, unsafe_allow_html=True)
                col4.markdown(f"""
                    <div style='font-size: 14px; color: gray; margin-top: 80px;'>급감속</div>
